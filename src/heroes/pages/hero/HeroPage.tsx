...
import useGetHero from "@/heroes/hooks/useGetHero"
import type { Key } from "react"

const HeroPage = () => {
  const { idSlug = ''} = useParams()

  const { data: superheroData, isError } = useGetHero(idSlug)
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {superheroData.powers.map((power:string, index: Key | null | undefined) => (
                    <div
                      key={index}
